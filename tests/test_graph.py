...
def test_reduce() -> None:
    ...
    graph = Graph.graph_from_iter('data').reduce(
        operations.FirstReducer(), ['test_id'])
    result = list(graph.run(data=lambda: iter(data)))
    assert expected == result
...
def test_join() -> None:
    ...
    expected = [{'game_id': 2, 'player_id': 1, 'score': 17, 'username': 'XeroX'},
                {'game_id': 3, 'player_id': 1, 'score': 22, 'username': 'XeroX'},
                {'game_id': 1, 'player_id': 3, 'score': 99, 'username': 'Destroyer'}]

    graph1 = Graph.graph_from_iter('data_left')
    graph2 = Graph.graph_from_iter('data_right')

    graph = graph1.join(operations.InnerJoiner(), graph2, ['player_id'])
    result = list(graph.run(
        data_left=lambda: iter(data_left),
        data_right=lambda: iter(data_right)))
...
def test_independence_of_word_count() -> None:
    ...
    result = list(graph.run(docs=lambda: iter(docs)))
    result2 = list(graph.run(docs=lambda: iter(docs)))
...
def test_independence_of_tf_idf() -> None:
    graph = algorithms.inverted_index_graph(
        'texts', doc_column='doc_id', text_column='text',
    ...
    result = list(graph.run(texts=lambda: iter(rows)))
    result2 = list(graph.run(texts=lambda: iter(rows)))

    assert sorted(result2, key=itemgetter('doc_id', 'text')) == expected
    assert result == result2
...
def test_independence_of_pmi() -> None:
    graph = algorithms.pmi_graph(
        'texts', doc_column='doc_id', text_column='text', result_column='pmi')
    ...
    rows = [
        ...
        {'doc_id': 5, 'text': 'HELLO HELLO! WORLD...'},
        {'doc_id': 6, 'text':
            'world? world... world!!! WORLD!!! HELLO!!! HELLO!!!!!!!'}
    ]
...
def test_independence_of_yandex_maps() -> None:
    graph = algorithms.yandex_maps_graph(
        'travel_time', 'edge_length', enter_time_column='enter_time',
    ...
    result = list(
        graph.run(
            travel_time=lambda: islice(cycle(iter(times)),
                                       len(times)),
    ...
    result2 = list(
        graph.run(
            ...
            edge_length=lambda: iter(lengths)))
    assert result == result2
    assert sorted(result2, key=itemgetter('weekday', 'hour')) == expected
